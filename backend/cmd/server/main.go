...
func main() {
	...
	lotSvc := service.LotService{Repo: repository.LotRepository{DB: db}, Kilns: repository.KilnRepository{DB: db}, Schedules: schedules, Audit: audit}
	readings := service.ReadingService{Repo: repository.ReadingRepository{DB: db}, Lots: repository.LotRepository{DB: db}, Schedules: schedules, Audit: audit}
	deps := router.Dependencies{Auth: auth, Kilns: handler.KilnHandler{Service: kilns}, Lots: handler.LotHandler{Service: lotSvc}, Readings: handler.ReadingHandler{Service: readings}, Schedules: handler.ScheduleHandler{Service: schedules}, Audit: handler.AuditHandler{Service: audit}}
