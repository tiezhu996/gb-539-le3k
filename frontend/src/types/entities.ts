...
export type ScheduleExpiryReason = 'reading_backfilled' | 'reading_voided' | 'reading_corrected';
...
export interface DryingSchedule {
  id:string; timber_lot_id:string; kiln_snapshot:string; algorithm_version:string; input_hash:string;
  stages_json:string; recommended_changes_json:string; predicted_finish_at:string|null; defect_risk_score:number;
  schedule_state:ScheduleLifecycle; explanation:string; calculated_at:string; created_by:string; reviewed_by:string; version:number;
  frozen_at?:string|null; frozen_by?:string;
  /** ISO timestamp when the reading basis changed and invalidated this plan. */
  expired_at?:string|null;
  expired_by?:string;
  expiry_reason?:ScheduleExpiryReason|'';
  /** Newer plan that replaced this one. */
  superseded_by_id?:string;
  /** Older plan this recalculated plan replaced. */
